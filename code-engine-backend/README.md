...
CodeEngine implements SHA-256 hashed API key authentication with a flat rate limit of **100 requests per key per 24 hours**.
...
*   **Response** (`201 Created`):
    ...
    ```
    > [!IMPORTANT]
    > The plaintext `api_key` is returned **only once** in this response and cannot be retrieved again. Only the SHA-256 hash is stored.

#### 2. Check Key Usage & Remaining Quota
*   **Method**: `GET /api/keys/:prefix/usage`
*   **Response** (`200 OK`):
    ```json
    {
      "requests_today": 12,
      "daily_limit": 100,
      "remaining": 88,
      "resets_at": "2026-07-26T14:00:00Z"
    }
    ```

#### 3. Revoke an API Key
*   **Method**: `DELETE /api/keys/:prefix`
*   **Response** (`200 OK`):
    ```json
    {
...
### Using API Keys (`curl` Quickstart)
...
#### Step 2: Submit Code for Sandboxed Execution
Pass your generated API key using the `X-API-Key` header:
```bash
curl -X POST http://localhost/submit \
  -H "Content-Type: application/json" \
  -H "X-API-Key: ce_YOUR_GENERATED_API_KEY" \
  -d '{
    "language": "python",
    "code": "print(\"Hello from CodeEngine!\")",
    "input": ""
  }'
```

#### Step 3: Stream Real-Time Execution Results
```bash
curl -N -H "X-API-Key: ce_YOUR_GENERATED_API_KEY" \
  http://localhost/stream/YOUR_SUBMISSION_ID
```
